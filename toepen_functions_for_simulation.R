...
endSet <- function(game, limit = 15){
  ...
  for(i in 1:game_i$players){ 
    ...
    if(game_i$player_points[[i]]$points >= limit){
      game_i$player_points[[i]]$dead <- 1
    }
  }
...
givePlayerOrder <- function(game, startPlayer = "dealer", excludePlayer = 0, exludeDeadPlayers = TRUE, exludePasPlayers = FALSE){
  ...
  if(startPlayer == "pas_over_dealer"){
    player_order <- c(1:game_i$players,1:game_i$players)
    start_player <- last(
                         c(which(sapply(game_i$player_points, function(player_i){  player_i$dealer == 1})),   ## dealer player 
                           which(sapply(game_i$round, function(player_i){  last(player_i$slag) == 1})))       ## player who won last slag
                     )
  ...
  if(!(any(startPlayer == c("dealer","pas_over_dealer")))){    ### none of these
    player_order <- c(1:game_i$players,1:game_i$players)
...
WhoWinsRound <- function(game){
  ...
  evaluation_df$suit <-
    sapply(evaluation_df$player, function(player) {
      if(length(game$round[[player]]$played_cards) == game$slag_nr){
        game$round[[player]]$played_cards[[game$slag_nr]]$suit
      } else {""}
  ...
  evaluation_df$value<-
    sapply(evaluation_df$player, function(player) {
      if(length(game$round[[player]]$played_cards) == game$slag_nr){
        game$round[[player]]$played_cards[[game$slag_nr]]$value
          ...
          if (game$round[[player_i]]$slag[length(game$round[[player_i]]$slag)] == 1) {
            ...
            if (sum(1 - sapply(1:game$players, function(i) {
              ...
              current_suit_by_slag_player <-
                ...
                #cards_by_slag_player$suit[length(cards_by_slag_player$suit)]
...
managePlayerChoices <- function(game, player, choices){
  ...
  if(gamePlay == "manual"){   ## manual player
    ...
    cat(">> kaarten in hand:", showHandOfPlayer(game, player), "<< \n")
    cat(formatChoices(choices = provideChoices(game, player), open = TRUE))
  ...
  if(gamePlay == "random_open"){             ## computer
    cat(">> kaarten in hand:", showHandOfPlayer(game, player), "<< \n")
    cat(formatChoices(choices = provideChoices(game, player), open = TRUE))
    choice <- chooseRandom(game, player, choices)
  }
  if(!(gamePlay %in% c("random","manual","random_open"))){               ## regard as a shell command
    ## write info file
    write_game_to_JSON(filename = paste("player",player,".txt",sep=""),
                       game = givePlayerEssentialInformation(game, player = player, choices = provideChoices(game, player))
    )
...
initializeNewSet <- function(game){
  ...
  game_i$player_cards <-     ## verdelen
    sapply(1:game_i$players, function(player_i) {
      list(cards =  game_i$deck[seq(1, 32, 4)[player_i]:seq(4, 32, 4)[player_i]],
           play_for_points = ifelse(player_i %in% activePlayers(game), 1,0),     ## for 1 if still active, otherwise 0
           wait_for_toep_response = 0,
           pas = 0,
           wants_to_change_cards = 0,
           play_with_this_hand = 0)
