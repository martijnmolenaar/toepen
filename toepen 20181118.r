...
game <- initializeGame(numberOfPlayers = 3,  player_names = c("Martijn", "Wouter","Ralph"),
                       gamePlay = c("Rscript randomAI.r -f player1.txt",
                                    "random", 
                                    "random"))
...
#game$player_points$Wouter$dead <- 1
#game$player_cards$Wouter$play_for_points <- 0
...
for(round_nr in 1:4){
  ...
  while( !all(sapply(activePlayers(game), function(i){length(game$round[[i]]$played_cards)}) == round_nr) &  ## nog minder kaarten gespeeld door actieve sperers dan het rondenr
    ...
    game$playing_order <- givePlayerOrder(game, exludePasPlayers = TRUE)
...
invisible(sapply(1:game$players, function(i){
  cat(game$player_names[i] ,": ",game$player_points[[i]]$points, " points\n",sep="")
  }))
